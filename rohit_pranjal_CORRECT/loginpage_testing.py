...
def login_student():
    if request.method == 'POST':
        ...
        return redirect(url_for('run_student_script'))
    return render_template('student.html')

@app.route('/run_student_script', methods=['POST'])
def run_student_script():
    subprocess.call(['python', 'student.py'])
    return ('', 204)  # Return HTTP status 204 (No Content) to indicate success
