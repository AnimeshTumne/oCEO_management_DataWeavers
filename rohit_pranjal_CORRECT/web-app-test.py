...
def student_personal_info():
    if "email" in session:
        ...
        return render_template('student/personal_info.html', student_data=fetched_data)
...
def student_jobs_available():
    if "email" in session:
        ...
        cursor.close()
        if request.method == 'POST':
            if request.form.get("submit_button") == 'Apply Job':
...
def student_apply_job():
    if "email" in session: 
        if request.method == 'POST': # submits the application  DOUBT
            cursor = db.connection.cursor()
            ...
            sql_app_id = f"INSERT INTO application_status (application_id, faculty_approved, oceo_coordinator_approved, SA_approved, dead_approved, statement of motivation, roll_number, job_id, approval) VALUES ({application_id}, 0, 0, 0, 0, {so_motivation}, {roll_number}, 101, 'pending');"
...
def student_applied_jobs():
    if "email" in session:
        ...
        cursor = db.connection.cursor()
        sql = f"SELECT roll_number FROM applied_student where email_id = '{session['email']}';"
        cursor.execute(sql)
...
def student_personal_info_change():
    if "email" in session:
        if request.method == 'POST':
            ...
            cursor = db.connection.cursor()
            cursor.execute(f"UPDATE applied_student SET cpi = {cpi}, last_sem_spi = {last_sem_spi}, on_probation = {on_probation} WHERE email_id = '{session['email']}';")
        ...
        return render_template('student/personal_info_change.html',student_data=fetched_data)
