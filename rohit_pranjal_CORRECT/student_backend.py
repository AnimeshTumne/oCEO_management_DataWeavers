...
def register():
    if request.method == "POST":
        ...
        if userType == "student":
            sql = f"SELECT count(*) FROM applied_student WHERE email='{email}'"
            cursor.execute(sql)
...
def authenticate(email, password):
    cursor = db.connection.cursor()
    sql = f"SELECT password FROM applied_student WHERE email='{email}'"
    cursor.execute(sql)
...
def login_student():
    if request.method == 'POST':
        ...
        cursor = db.connection.cursor()
        sql = f"SELECT roll_number FROM applied_student WHERE email='{email}'"
        cursor.execute(sql)
...
def student_personal_info_change():
    if "roll_number" in session:
        if request.method == 'POST':
            ...
            cursor.execute(f"UPDATE applied_student SET first_name = '{first_name}', middle_name = '{middle_name}', last_name='{last_name}',cpi = {cpi}, last_sem_spi = {last_sem_spi}, on_probation = {on_probation} WHERE roll_numer = {roll_number};")
            db.connection.commit()
            ...
            if phone_num_exist(roll_number):
                ...
                cursor.execute(f"UPDATE applied_student_phone SET phone_number = {phone_1} WHERE roll_number = {roll_number};")
                db.connection.commit()
                if phone_2!="": 
                    cursor.execute(f"UPDATE applied_student_phone SET phone_number = {phone_2} WHERE roll_number = {roll_number};")
                    db.connection.commit()
            else:
        ...
        return render_template('student/personal_info_change.html', student_data = fetched_data, sttudent_phone = fetched_phone)
...
def student_apply_job():
    if "roll_number" in session:
        ...
        job_ids = [job[0] for job in unapplied_jobs]
        ...
        for job in unapplied_jobs:
            if job[1]=="ADH": 
                ...
                job_roles.append(f"ADH: {course_id} - {course_name}")
            elif job[1]=="PAL": 
                cursor.execute(f"SELECT subjects_to_teach FROM subjects_under_pal WHERE job_id = {job[0]};")
                subjects_to_teach = cursor.fetchone()[0]
                job_roles.append(f"PAL: {subjects_to_teach}")
            else:
                cursor.execute(f"SELECT role_name FROM others WHERE job_id = {job[0]};")
                role_name = cursor.fetchone()[0]
                job_roles.append(f"{role_name}")                

        cursor.close()

        jobs_data = list(zip(job_ids, job_roles))

        return render_template('student/apply_job.html', jobs = jobs_data) 
